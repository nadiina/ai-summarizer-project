...
from libs.logger import log_info, log_warning, log_error
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    log_info(f"Chat {chat_id}: отримано файл {doc.file_name}")

    if not allowed_file(doc.file_name):
        log_warning(f"Chat {chat_id}: невірний формат файлу {doc.file_name}")
    ...
    # log_info(f"Chat {chat_id}: файл збережено за шляхом {file_path}")
...
async def handle_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    action = query.data
    # log_info(f"Chat {chat_id}: обрано дію {action}")

    file_info = user_files.get(chat_id)
    if not file_info:
        log_error(f"Chat {chat_id}: файл не знайдено для дії {action}")
    ...
    try:
        with open(file_info["path"], 'rb') as f:
            ...
            log_info(f"Chat {chat_id}: надсилаю POST на {url}")
            response = requests.post(url, files=files)
            log_info(f"Chat {chat_id}: отримано відповідь API {response.status_code}")

        if response.status_code == 200:
            data = response.json()
            result_key = "summary" if action == "get_summary" else "contents"
            result = data.get(result_key, "Результат порожній.")
            # log_info(f"Chat {chat_id}: успішно отримано результат для {action}")
            ...
            log_error(f"Chat {chat_id}: API повернув помилку {response.status_code} - {response.text}")
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=processing_msg.message_id,
        ...
        log_error(f"Chat {chat_id}: помилка під час обробки")
        await context.bot.edit_message_text(
            chat_id=chat_id,
            message_id=processing_msg.message_id,
